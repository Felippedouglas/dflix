...
export default function Pesquisar() {
    ...
    function pesquisarPalavraSugerida(palavraSugerida) {
        setName(palavraSugerida);
    }
            ...
            <header className="header-componente-pesquisar">
                <section className="section-pesquisar">
                    <div className="section-input-pesquisar">
                        <input autoComplete="off" minLength={3} maxLength={30} onFocus={()=>exibirListaSugestoesNomesPesquisar()} onBlur={()=>esconderListaSugestoesNomesPesquisar()} autoFocus type='text' id="input-pesquisar" value={name} placeholder='Pesquise por filmes e séries' onChange={(e)=>definirFilmePesquisado(e.target.value)}/>
                        <a onClick={()=>recarregarPagina()} id='bt-pequisar-movie'><i className="fas fa-search"></i></a>
                    </div>
                    <div style={{display: exibirListaSugestoesPesquisar ? 'flex' : 'none'}} className="div-sugestoes-pesquisar">
                        {
                            <ul>
                                <Link to={`/pesquisar/search=${filmePesquisado}&pagina=1`} onClick={()=>pesquisarPalavraSugerida(filmePesquisado)}><i className="fas fa-search"></i> {filmePesquisado}</Link>
                        ...
                        <Link className="link-pesquisar-sugestao-movie" to={`/pesquisar/search=${filmePesquisado}&pagina=1`}>Ver tudo</Link>
                    </div>
